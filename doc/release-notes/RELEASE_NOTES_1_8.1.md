...
Crypton Core 1.8.1 is primarily a bugfix release, bringing Crypton Core in
line with Bitcoin 0.9.3. Crypton Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Crypton Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Crypton Core clients
* Use HTTPS with cryponchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/crypton-dev/crypton/issues/134).
...
This only affects values significantly below 1 CRN, however merchants may wish to round to
the nearest CRN on all transactions for now.
