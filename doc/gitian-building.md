...
*Setup instructions for a gitian build of Crypton using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Crypton
...
- [Building Crypton](#building-crypton)
...
echo 'crntl addbr br0' >> /etc/rc.local
...
Clone the git repositories for crypton and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/crypton-dev/crypton
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the crypton repository
...
To build Crypton (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the crypton repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/crypton/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/crypton-dev/crypton
...
For example:
```bash
URL=https://github.com/laanwj/crypton.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit crypton=${COMMIT} --url crypton=${URL} ../crypton/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit crypton=${COMMIT} --url crypton=${URL} ../crypton/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit crypton=${COMMIT} --url crypton=${URL} ../crypton/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/crypton-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/crypton-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/crypton-osx-build.assert
```
...
[crypton/gitian.sigs](https://github.com/crypton/gitian.sigs/) repository, or if that's not possible create a pull request. 
