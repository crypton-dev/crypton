...
Below are some notes on how to build Crypton Core for Windows.

The options known to work for building Crypton Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Crypton Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Crypton Core applications.
...
Note that for WSL the Crypton Core source path MUST be somewhere in the default mount file system, for
example /usr/src/crypton, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/crypton-dev/crypton.git
    sudo chmod -R a+rw crypton
...
Once the source code is ready the build steps are below.
    ...
    CONFIG_SITE=$PWD/depends/x86_64-w64-mingw32/share/config.site ./configure --prefix=/usr/src/crypton/depends/x86_64-w64-mingw32/
    make
...
The next three steps are an example of how to acquire the source in an appropriate way.
    ...
    cd /usr/src
    sudo git clone https://github.com/crypton-project/crypton.git
    sudo chmod -R a+rw crypton
...
Then build using:
    ...
    cd ..
    ./autogen.sh # not required when building from tarball
    CONFIG_SITE=$PWD/depends/i686-w64-mingw32/share/config.site ./configure --prefix=/usr/src/crypton/depends/x86_64-w64-mingw32/
    make
...
way. This will install to `c:\workspace\crypton`, for example:

    make install DESTDIR=/mnt/c/workspace/crypton
...
causes two of the crypton executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the crypton code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the crypton source code).
