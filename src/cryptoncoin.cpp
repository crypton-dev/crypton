...
// Crypton: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateCryptonNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetCryptonBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, uint256 prevHash)
{
    if ((nHeight > 6000) && (nHeight < 17113)){
...
int64_t GetCryptonDustFee(const std::vector<CTxOut> &vout, CFeeRate &baseFeeRate) {
    int64_t nFee = 0;
