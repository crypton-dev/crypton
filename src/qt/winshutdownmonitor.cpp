...
{
    typedef BOOL (WINAPI *PSHUTDOWNCRNREATE)(HWND, LPCWSTR);
    PSHUTDOWNCRNREATE shutdownCRNreate = (PSHUTDOWNCRNREATE)GetProcAddress(GetModuleHandleA("User32.dll"), "ShutdownBlockReasonCreate");
    if (shutdownCRNreate == NULL) {
        qWarning() << "registerShutdownBlockReason: GetProcAddress for ShutdownBlockReasonCreate failed";
        return;
    }

    if (shutdownCRNreate(mainWinId, strReason.toStdWString().c_str()))
