...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or crypton.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Crypton is not connected!");

    if (IsInitialBlockDownload()) {
        cout << "IsInitialBlockDownload" << endl;
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Crypton is downloading blocks...");
    }
...
Value getauxblockbip22(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty() && !Params().MineBlocksOnDemand())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                           "Crypton is not connected!");

    if (IsInitialBlockDownload() && !Params().MineBlocksOnDemand())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Crypton is downloading blocks...");
